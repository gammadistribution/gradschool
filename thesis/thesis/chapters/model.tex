...
\begin{section}{Preliminaries}
  ...
  \begin{subsection}{Units of Analysis and Observation}
    ...
    the performance of the forecasting model. As mentioned in Section \ref{data:used}, the units of observation for this model
  ...
  \begin{subsection}{Covariates}
    ...
    the program airing %\cite{Danaher et al.}.
    ...
    for a detailed outline of the covariates that will be used. Binary covariates are ``shifted to have a mean of 0 and to differ by 1 in their upper and lower conditions'' per Gelman et al.%\cite BDA3
    ...
    \item Lead-in Content -
      ...
      by allowing for random effects based off the of the content identifier that aired immediately prior to the unit of observation.
...
\begin{section}{Description}
  ...
  is common throughout the literature. %$\cite{BDA 3, Doing Bayesian Data analysis}.
...
\begin{section}{Inference}
  ...
  \begin{subsection}{Convergence}
    ...
    \end{align}
    and we can check for approximate convergence by determining if $\hat{R}$ is close to 1. % cite BDA 3
    ...
    \end{align}
    where $\rho_t$ is the autocorrelation of the simulation sequence at lag $t$. % cite BDA 3.
...
\begin{section}{Validation}
  ...
  \begin{subsection}{Test Statistics}
    ...
    \begin{sidewaystable}[!htbp]
      ...
      \end{tabular}
        \caption{Example program table data for two historical program airings.}\label{tab:bayesp}
    \end{sidewaystable}
  \end{subsection}
  ...
  \begin{subsection}{Regression Fit}
    The last check that we will perform is the analysis of the regression fit of the
    model.
  \end{subsection}
