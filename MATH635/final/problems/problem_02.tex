\begin{problem}
  ...
  using the first $j=2,3,4$ basis functions. Plot
...
\begin{proof}
  ...
  \]
  then $y_n'' - y_n + x = 0$, i.e\ the approximation satisfies the differential
  equation \eqref{diffeq}. If $y_n(x)$ satisfies the
  differential equation and the boundary conditions, then we know that $y_n(x)$ approximates the
  exact solution $y(x)$.

  Therefore, we choose the coefficients $a_k$ such that they satisfy the system of
  equations
  \begin{align*}
